...
function CardTile({ card, selected, onClick }) {
 ...
 return (
   <div onClick={onClick} title={`${card.name} — L${card.level||"?"}`} style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:3,padding:"6px 4px",background:selected?`${col}25`:"rgba(255,255,255,0.03)",border:`1.5px solid ${selected?col:"rgba(255,255,255,0.07)"}`,borderRadius:8,cursor:"pointer",transition:"all 0.15s",boxShadow:selected?`0 0 12px ${col}55`:"none",position:"relative",minHeight:70}}>
     {card.iconUrls?.medium?<img src={card.iconUrls.medium} alt={card.name} style={{width:38,height:38,objectFit:"contain"}} onError={e=>{e.target.style.display="none"}}/>:<div style={{fontSize:20}}>{TYPE_ICONS[card.type?.toLowerCase()]||"🃏"}</div>}
...
function DeckCard({ card, onRemove }) {
 ...
 return (
   <div onClick={onRemove} style={{aspectRatio:"3/4",background:`linear-gradient(145deg,${col}20,${col}08)`,border:`1.5px solid ${col}55`,borderRadius:8,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:3,gap:2,position:"relative"}}>
     {card.iconUrls?.medium?<img src={card.iconUrls.medium} alt={card.name} style={{width:"68%",objectFit:"contain"}} onError={e=>{e.target.style.display="none"}}/>:<div style={{fontSize:16}}>{TYPE_ICONS[card.type?.toLowerCase()]||"🃏"}</div>}
     ...
     <div style={{position:"absolute",top:2,right:2,color:"rgba(255,255,255,0.18)",fontSize:9}}>✕</div>
   </div>
 );
}
...
function Bubble({ msg }) {
 ...
 return (
   <div style={{display:"flex",flexDirection:isAI?"row":"row-reverse",gap:8,alignItems:"flex-start",marginBottom:10}}>
     {isAI&&<div style={{width:28,height:28,borderRadius:"50%",background:"linear-gradient(135deg,#ff6f00,#e65100)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,flexShrink:0}}>🤖</div>}
     ...
     <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8,gap:8}}>
       <div style={{minWidth:0}}>
         <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3,flexWrap:"wrap"}}>
           ...
           {isLive&&<div style={{fontSize:8,background:"rgba(76,175,80,0.15)",border:"1px solid rgba(76,175,80,0.3)",borderRadius:3,padding:"1px 5px",color:"#81c784"}}>🔴 LIVE</div>}
         </div>
         <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
           ...
           <div style={{fontSize:11,color:ratingColor,fontWeight:700}}>⭐ {deckData.powerRating}/100</div>
           {deckData.winRate!=null&&<div style={{fontSize:11,color:"#4caf50",fontWeight:700}}>📊 {deckData.winRate}% WR</div>}
       ...
       {deckData.cards.map((cardName,i)=>{
         ...
         return (
           <div key={i} style={{aspectRatio:"3/4",background:card?`linear-gradient(145deg,${rcol}20,${rcol}08)`:"rgba(255,50,50,0.1)",border:`1px solid ${card?rcol+"55":"rgba(255,50,50,0.3)"}`,borderRadius:6,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:2,gap:1,position:"relative"}}>
             {card?.iconUrls?.medium?<img src={card.iconUrls.medium} alt={card.name} style={{width:"80%",objectFit:"contain"}} onError={e=>{e.target.style.display="none"}}/>:<div style={{fontSize:14}}>{card?TYPE_ICONS[card.type?.toLowerCase()]||"🃏":"❌"}</div>}
     ...
     {explanation?(
       <>
         <div style={{fontSize:11,color:"#888",marginBottom:8,padding:"6px 8px",background:"rgba(255,255,255,0.02)",borderRadius:6}}>
           🎯 <span style={{color:"#ccc"}}>{explanation.winCondition}</span>
         </div>
         {explanation.synergies?.length>0&&(
           <div style={{marginBottom:8}}>
             <div style={{fontSize:9,color:col,fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>⚡ KEY SYNERGIES</div>
         ...
         <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
           <div style={{background:"rgba(255,111,0,0.05)",border:"1px solid rgba(255,111,0,0.15)",borderRadius:6,padding:"7px 9px"}}>
             <div style={{fontSize:9,color:"#ff9a40",fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:3}}>⚔️ ATTACK</div>
           ...
           <div style={{background:"rgba(33,150,243,0.05)",border:"1px solid rgba(33,150,243,0.15)",borderRadius:6,padding:"7px 9px"}}>
             <div style={{fontSize:9,color:"#64b5f6",fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:3}}>🛡️ DEFENSE</div>
             <div style={{fontSize:10,color:"#666",lineHeight:1.5}}>{explanation.defense}</div>
           </div>
         </div>
         {explanation.counters?.length>0&&(
           <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center",marginBottom:8}}>
             <span style={{fontSize:9,color:"#555",textTransform:"uppercase",letterSpacing:1}}>⚠️ Watch out:</span>
         ...
         {explanation.upgradePriority?.length>0&&(
           <div style={{borderTop:"1px solid rgba(255,255,255,0.04)",paddingTop:8}}>
             <div style={{fontSize:9,color:"#ffd700",fontWeight:700,textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>⬆️ UPGRADE PRIORITY</div>
...
export default function App() {
 ...
 const [messages, setMessages] = useState([{role:"assistant",content:"👑 Enter your #PLAYERTAG and hit LINK — I'll check live battle data to find your best decks with real win rates!"}]);
 ...
 const selectDeck = (deckData) => {
   ...
   setMessages(prev=>[...prev,
     ...
     {role:"assistant",content:`🔥 Locked in! ${deckData.tier} tier · ${deckData.powerRating}/100${deckData.winRate!=null?` · ${deckData.winRate}% WR`:""}\n\n${expl?.winCondition||"Use your win condition aggressively!"}\n\n⚡ ${expl?.synergies?.[0]||"Good luck!"}\n\nAsk me anything!`}
   ]);
 };
 ...
 return (
   <div style={{minHeight:"100vh",background:"#08080f",color:"#e0e0e0",fontFamily:"'Rajdhani','Oswald',sans-serif",display:"flex",flexDirection:"column"}}>
     ...
     <div style={{background:"linear-gradient(90deg,#140800,#0a0a1a,#140800)",borderBottom:"1px solid rgba(255,111,0,0.15)",padding:"10px 16px",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap",boxShadow:"0 2px 20px rgba(255,111,0,0.08)"}}>
       <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
         ...
         <div>
           ...
           <div style={{fontSize:9,color:"#333",letterSpacing:1,textTransform:"uppercase"}}>Live Data · Synergy Engine · AI Coach</div>
         </div>
       </div>
         ...
         {player?(
           <div style={{display:"flex",alignItems:"center",gap:8,background:"rgba(255,111,0,0.07)",border:"1px solid rgba(255,111,0,0.2)",borderRadius:8,padding:"6px 10px",flex:1}}>
             ...
             <div style={{flex:1,minWidth:0}}>
               ...
               <div style={{fontSize:9,color:"#444"}}>{player.trophies?.toLocaleString()} trophies · {player.arena?.name} · {allCards.length} cards</div>
             </div>
     ...
     {fetchStatus&&<div style={{background:"rgba(255,111,0,0.06)",borderBottom:"1px solid rgba(255,111,0,0.12)",color:"#cc7a30",padding:"5px 16px",fontSize:11}}>⏳ {fetchStatus}</div>}
     {error&&<div style={{background:"rgba(255,50,50,0.07)",borderBottom:"1px solid rgba(255,50,50,0.15)",color:"#ff5252",padding:"5px 16px",fontSize:11}}>⚠️ {error}</div>}
     {dataSource&&!fetchStatus&&(
       <div style={{background:dataSource==="live"?"rgba(76,175,80,0.06)":"rgba(255,111,0,0.04)",borderBottom:`1px solid ${dataSource==="live"?"rgba(76,175,80,0.15)":"rgba(255,111,0,0.1)"}`,color:dataSource==="live"?"#66bb6a":"#555",padding:"4px 16px",fontSize:10,display:"flex",gap:16,flexWrap:"wrap"}}>
         <span>{dataSource==="live"?`🔴 LIVE — ${dbMeta?.battles?.toLocaleString()||"?"} battles · ${dbMeta?.totalDecks?.toLocaleString()||"?"} unique decks`:"📋 META ENGINE — Live DB building, will switch to real data automatically"}</span>
         ...
         <div style={{flex:1,overflowY:"auto",padding:"0 12px 12px"}}>
           {!player?(
             <div style={{textAlign:"center",color:"#1e1e1e",fontSize:12,paddingTop:28}}>
               <div style={{fontSize:26,marginBottom:6}}>🔗</div>Link your account<br/>to see your cards
             </div>
           ):(
           ...
           <div style={{flex:1,overflowY:"auto",padding:20}}>
             <div style={{maxWidth:700,margin:"0 auto"}}>
               ...
               <div style={{color:"#333",fontSize:11,marginBottom:16}}>
                 {dataSource==="live"?"Ranked by real win rate from live battle data":"Ranked by meta score + synergy + card levels"} · Hit USE THIS to auto-fill
               </div>
               {!player?(
                 <div style={{textAlign:"center",color:"#222",paddingTop:40,fontSize:13}}>
                   <div style={{fontSize:32,marginBottom:8}}>🔗</div>Link your account
                 </div>
               ):deckOptions.length===0?(
                 <div style={{textAlign:"center",color:"#333",paddingTop:40,fontSize:13}}>
                   <div style={{fontSize:32,marginBottom:8}}>⏳</div>{fetchStatus||"Loading..."}
                 </div>
               ):(
                 deckOptions.map((d,i)=>(
         ...
         {tab==="build"&&(
           <div style={{flex:1,overflowY:"auto",padding:24}}>
             <div style={{maxWidth:540,margin:"0 auto"}}>
               ...
               <div style={{color:"#2a2a2a",fontSize:13,marginBottom:20}}>Link → Pick deck → Get AI coached</div>
               {!player?(
                 <div style={{background:"rgba(255,111,0,0.05)",border:"1px solid rgba(255,111,0,0.14)",borderRadius:10,padding:18}}>
                   ...
                   {["Enter your #PLAYERTAG above and hit LINK","Checks live battle database for real win rates","Falls back to meta engine if DB is still building","Go to ⚡ DECKS — pick your best deck","Hit USE THIS — slots auto-fill instantly","Chat with AI for coaching, counters and tips"].map((s,i)=>(
                 ...
                 <div style={{background:"rgba(46,125,50,0.06)",border:"1px solid rgba(46,125,50,0.16)",borderRadius:10,padding:14}}>
                   <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:13,color:"#66bb6a",letterSpacing:1,marginBottom:10}}>✅ {player.name?.toUpperCase()} — LINKED</div>
                   <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
                     {[["🏆","Trophies",player.trophies?.toLocaleString()],["🗺️","Arena",player.arena?.name],["🃏","Cards",allCards.length],["⭐","King Lvl",player.expLevel],["🏅","Best",player.bestTrophies?.toLocaleString()],["⚔️","Wins",player.wins?.toLocaleString()]].map(([icon,label,val])=>(
             ...
             <div style={{flex:1,overflowY:"auto",padding:"14px 18px"}}>
               ...
               {chatLoading&&(
                 <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:10}}>
                   <div style={{width:28,height:28,borderRadius:"50%",background:"linear-gradient(135deg,#ff6f00,#e65100)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13}}>🤖</div>
